...
def read_conll_file(file_path):
    """Read CoNLL format file from Label Studio export"""
    current_tokens = []
    current_labels = []
    all_tokens = []
    all_labels = []
    ...
    return pd.DataFrame({
        'tokens': all_tokens,
        'ner_tags': [[label2id[label] for label in labels] for labels in all_labels]
    })
    ...
    for i, label in enumerate(examples["ner_tags"]):
        ...
        for word_idx in word_ids:
            if word_idx is None:
                ...
                # For tokens that are part of the same word, copy the label
                label_ids.append(-100)
...
def train_roberta_ner(conll_file_path):
    """Train RoBERTa NER model using CoNLL format data"""
    ...
    df = read_conll_file(conll_file_path)
